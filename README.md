...
1. Put `showeverything-1.0.3-forge-1.19.2.jar` in the server `mods` folder.
...
Use the official Forge `1.19.2-43.4.4` MDK wrapper:

```bash
./gradlew clean build
```

The compiled mod jar is written to `build/libs/showeverything-1.0.3-forge-1.19.2.jar`.

## Compatibility

- The server and client must both use Minecraft/Forge 1.19.2 when the client mod is installed; no cross-version protocol compatibility is intended.
